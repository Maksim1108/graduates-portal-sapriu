...
// Simple stats for Admin dashboard (Vue client)
...
// ===== Users management for Vue Admin =====
...
exports.deleteUser = async (req, res, next) => {
    try {
        const { id } = req.params
        // Remove account and related form and gallery entries
        await Promise.all([
...
// Get user profile/forms for admin
...
// Get users with profile status information
...
exports.verifyProfile = async (req, res, next) => {
    try {
        ...
        // Update user role based on verification status, but preserve admin role
        const user = await accountModel.Account.findById(id)
        if (user && user.role !== 'admin') {
